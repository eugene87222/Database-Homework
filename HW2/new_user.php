<?php
	...
	if($_SERVER["REQUEST_METHOD"]=="POST"){
  		if(empty($_POST["username"])||strip_tags($_POST["username"])==''){
  		...
  		if(empty($_POST["name"])||strip_tags(trim($_POST["name"]))==''){
		...
		if($stmt->rowCount()==0){
			...
			echo<<<EOT
				...
				<script>
					...
					window.location.replace('member_management.php')
				</script>
				</body>
				</html>
EOT;		
...
<body class="register">
	...
	<div class="content">
		...
		<input onclick="window.location.replace('member_management.php');" type="submit" value="Cancel" class="general_button">
	</div>
	<?php
			}
			else{
