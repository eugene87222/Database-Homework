...
EOT;
	...
	if(isset($_POST["submit"])){
		if($_POST["submit"]=="delete"){
			$ID=$_POST["ID"];
			$del=$connect->prepare("DELETE FROM location WHERE ID=:ID");
	...
	<div class="content">
		<nav>
			<ul>
				...
				<li><form>
					<input type="submit" onClick="ShowPopup('new_location.php')" value="新增地點">
		...
		<table id="loca_info_mgnt">
			<tr class="caption">
				...
				<?php
					$sql="SELECT ID,location FROM location ORDER BY ID";
					$getdata=$connect->prepare($sql);
					$getdata->execute();
					$number_of_result=$getdata->rowCount();
					$number_of_page=ceil($number_of_result/$result_per_page);
					$sql="SELECT ID,location FROM location ORDER BY ID LIMIT ".($page-1)*$result_per_page.",".$result_per_page;
					$getdata=$connect->prepare($sql);
					$getdata->execute();
					while($location=$getdata->fetch()){
         		?>
      		<tr class="data">
          		<th><?php echo $location['ID'];?></th>
          		<th class="middle"><?php echo $location['location'];?></th>
          		<th class="end">
          			<form class="modify" action=<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?> method="post">
          				<input onclick="return confirm('Are you sure you want to delete this location ?')" type="submit" name="submit" value="delete" id="delete">
		...
		<?php
			for($page=1;$page<=$number_of_page;$page++){
				echo '<a href="location_management.php?page='.$page.'">'.$page.'</a>&nbsp;&nbsp;';
			}
		?>
	</div>
	<br>
